          ...
          <div class="col-lg-8">
            <div class="card ">
              ...
              <?php if (isset($_REQUEST['q_id'])) {
                ...
                $sqli = $link->query("SELECT * FROM `response` WHERE  student_id='$student_id' AND type='que_ans' AND id=$q_id");
              }else {
              $sqli = $link->query("SELECT * FROM `response` WHERE  student_id='$student_id' AND type='que_ans'");
                }
             if ($sqli->num_rows > 0) {
               ...
               ?>
               <?php
               $sq = $link->query("SELECT id FROM `response` WHERE id = (select max(id) from response where id < '$c_id') AND  student_id='$student_id' AND type='que_ans' ");
               if ($sq->num_rows > 0) {
                 $pre = $sq->fetch_assoc();
                 $pre_btn=$pre['id'];
               }
                ...
                ?>
              <div class="card-body">
                <h3 class="text-danger"> <?php echo $data['question']; ?></h3>
                <div class="float-right">
                  <?php echo $data['mark']." "."marks" ?>
                </div>
                    <form role="form" action="" method="post">
                    <label> Write your answer</label>
                  <textarea name="ans"  id="myInput" class="form-control"  rows="15" ><?php echo $data['answer']; ?></textarea>
                ...
                <div class="float-right">
                  ...
                  $sql = $link->query("SELECT id FROM `response` WHERE id = (select min(id) from response where id > '$c_id') AND  student_id='$student_id' AND type='que_ans' ");
                  if ($sql->num_rows > 0) {
            ...
            <div class="card">
                ...
                <center>
                  <div class="card-body">
                    <?php
                    $count=0;
                    $sqli =" SELECT * FROM `response` WHERE student_id ='$student_id' AND type='que_ans'" ;
                    $data = $link->query($sqli);
                    if($data->num_rows>0){
                         while ($row = $data->fetch_assoc()) {
                           $count++;
                    ?>
                  <a href="?id=<?php echo base64_encode($student_id); ?>&q_id=<?php echo base64_encode($row['id']); ?>" class="btn btn-app <?php if ($row['answer']!="") {
                    echo "bg-green";
